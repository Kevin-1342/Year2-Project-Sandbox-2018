...
void KinectProjector::CalibrateNextPoint()
{
	...
	else
	{
		...
		{ // We ask for higher points
			calibModal->hide();
			confirmModal->show();
			confirmModal->setMessage("Cover the sandbox with a board.");
...
void KinectProjector::askToFlattenSand(){
    ...
    ofBackground(255);
    fboProjWindow.end();
    confirmModal->setMessage("Surface calibration");
    confirmModal->show();
...
void KinectProjector::setupGui(){
	...
	gui->addButton("start Sandbox")->setName("Start Application");
	gui->addBreak();
    gui->addFRM();
